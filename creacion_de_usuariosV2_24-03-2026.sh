...
echo -n "¿El usuario tendrá permisos sudo? (s/n): "
read sudo_opcion

if [ "$sudo_opcion" = "s" ]; then
    echo "¿Qué comandos podrá ejecutar?"
    echo "Ejemplo: /usr/bin/apt, /usr/bin/systemctl"
    echo "Escribe ALL para acceso completo"
    echo -n "Comandos: "
    read comandos

    archivo_sudo="/etc/sudoers.d/$user"

    echo "$user ALL=(ALL) $comandos" > "$archivo_sudo"
    chmod 440 "$archivo_sudo"

    echo "Permisos sudo asignados correctamente."
fi
